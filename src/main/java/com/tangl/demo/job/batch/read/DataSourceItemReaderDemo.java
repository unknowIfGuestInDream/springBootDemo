...
public class DataSourceItemReaderDemo {
    ...
    //在数据库中新建一张TEST表，SQL语句如text.sql文件
    //表的字段和上面创建的TestData实体类一致
    /*
    dataSourceItemReader()方法中的主要步骤就是：通过JdbcPagingItemReader设置对应的数据源，
    然后设置数据量、获取数据的sql语句、排序规则和查询结果与POJO的映射规则等。
    方法末尾之所以需要调用JdbcPagingItemReader的afterPropertiesSet()方法是因为需要设置JDBC模板
     */
    @Bean
